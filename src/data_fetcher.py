...
def fetch_stock_data(ticker: str) -> Dict[str, Any]:
    ...
    for _, row in hist.iterrows():
        try:
            record = RawPriceData(
                Date=row['Date'],
                Open=Decimal(str(row['Open'])) if not pd.isna(row['Open']) else Decimal('0'),
                High=Decimal(str(row['High'])) if not pd.isna(row['High']) else Decimal('0'),
                Low=Decimal(str(row['Low'])) if not pd.isna(row['Low']) else Decimal('0'),
                Close=Decimal(str(row['Close'])) if not pd.isna(row['Close']) else Decimal('0'),
                Volume=int(row['Volume']) if not pd.isna(row['Volume']) else 0
            )
            price_records.append(record)
        except Exception as e:
            logger.warning(f"Skipping invalid price row for {ticker} on {row.get('Date')}: {e}")
    ...
    if fundamental_source in ["quarterly", "annual"]:
        ...
        for report_date, row in balance_sheet.iterrows():
            rec = RawFundamentalData(
                Date=report_date,
                TotalAssets=Decimal(str(row.get('Total Assets', 0))) if pd.notna(row.get('Total Assets')) else None,
                TotalLiab=Decimal(str(row.get('Total Liab', 0))) if pd.notna(row.get('Total Liab')) else None,
                ShareholderEquity=Decimal(str(row.get('Total Stockholder Equity', 0))) if pd.notna(row.get('Total Stockholder Equity')) else None,
                SharesOutstanding=int(info.get('sharesOutstanding', 0)) if info.get('sharesOutstanding') else None,
                MarketCap=Decimal(str(info.get('marketCap', 0))) if info.get('marketCap') else None,
                EnterpriseValue=Decimal(str(info.get('enterpriseValue', 0))) if info.get('enterpriseValue') else None
            )
